...
namespace EncryptDemo
{
    ...
    public sealed class HttpUtil
    {
        ...
        public string PostRequest(string url, IDictionary<string, string> parameters,bool isParams)
        {
            var cookieContainer = new CookieContainer();
            if (isParams)
                return PostRequest(url, parameters, ref cookieContainer);
            else
                return PostRequestObject(url, parameters, ref cookieContainer);
        }
        ...
        /// <returns></returns>
        public string PostRequestObject(string url, IDictionary<string, string> parameters, ref CookieContainer cookieContainer)
        {
            HttpWebRequest req = GetWebRequest(url, "POST");
            if (cookieContainer != null && cookieContainer.Count > 0)
            {
            ...
            byte[] postData = Encoding.UTF8.GetBytes(BuildQueryJsonStr(parameters));
            System.IO.Stream reqStream = req.GetRequestStream();
            reqStream.Write(postData, 0, postData.Length);
            reqStream.Close();
        ...
        /// <param name="parameters">Key-Value形式请求参数字典</param>
        /// <returns>URL编码后的请求数据</returns>
        public static string BuildQueryJsonStr(IDictionary<string, string> parameters)
        {
