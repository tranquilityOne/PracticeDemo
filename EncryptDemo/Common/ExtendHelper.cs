...
namespace EncryptDemo
{
    public static class ExtendHelper
    {
        ...
        public static String ToHexPadLeft(this byte value, int length)
        {
            return value.ToString("X").PadLeft(length, '0');
        }
        ...
        /// byte 转 Int
        /// </summary>
        /// <param name="byts">数组</param>
        /// <param name="sartIndex">开始索引</param>
        /// <param name="lastIndex">结束索引</param>
        /// <returns></returns>
        public static int ByteToInt32(this byte[] bytes, int startIndex, int len)
        {
            string hexStr = string.Empty;
            for (int i = startIndex; i < startIndex + len; i++)
            {
                hexStr += bytes[i].ToHexPadLeft(2);
            }
